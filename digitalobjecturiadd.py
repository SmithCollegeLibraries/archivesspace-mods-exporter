...
with open("ywcadigitalobjects.json") as json_file:
    try:
...
def updateDOwithCompassURI(ywca_compass_objects):
	' Updates ArchivesSpace Digital Object records with their Compass URIs '
	
	for compass_object in ywca_compass_objects:
		# Gets the digital object id and the digital object record
		do_id = compass_object['mods_identifier_local_s']
		digital_object = aspace.get('/repositories/2/digital_objects/' + do_id)
		# Gets the Compass URI and builds the data dictionary necessary for the API call updating the digital object record
		compass_uri = compass_object['PID']
...
## -- Function call -- ##
# updated_dos = updateDOwithCompassURI(ywca_compass_objects)


## ===== TEST STUFF (Ignore) ===== ##

# test = ywca_compass_objects[2]

# URI of Test:
# https://compass.fivecolleges.edu/object/islandora:22105
