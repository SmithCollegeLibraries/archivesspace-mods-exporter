...
if __name__ == "__main__":
    ...
    # note = 'Information about these clubs may be found with the departmental records of the specific department in <title render="bold">52. Departments</title> of the classification system. The types of material vary from club to club, but may include charters, flyers, photographs, programs, articles, zines, buttons, and notes….'
    # note = html.escape(note)
    # pp(note)
    # pp(html.unescape(note))
    ...
    # for c in cache:
    #         if c['digital_object']['create_time'].split('T')[0] == '2020-08-21':
    #             if not c['digital_object']['uri'] in sieve:
    #                 sieve.append(c['digital_object']['uri'])
    #                 compass_dos.append(c) 

    compass_dos = []
    do = aspace.client.get('/repositories/4/digital_objects/3651').json()
    do_obj = {}
    do_obj['digital_object'] = do
    do_obj['resource'] = aspace.client.get('/repositories/4/resources/134').json()
    do_obj['archival_object'] = aspace.client.get('/repositories/4/archival_objects/208136').json()
    compass_dos.append(do_obj)
